...
migrateup1:
	migrate -path db/migration -database "postgresql://postgres:postgrespw@localhost:5432/simple_bank?sslmode=disable" -verbose up 1
migratedown:
...
migratedown1:
	migrate -path db/migration -database "postgresql://postgres:postgrespw@localhost:5432/simple_bank?sslmode=disable" -verbose down 1
sqlc:
	sqlc generate
test:
	go test -v -cover ./...
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/ohno104dev/simplebank/db/sqlc Store
